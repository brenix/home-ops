...
| `nodes/<role>/<node>.yaml.j2`           | Per-node documents (hostname, addresses, zone)                            |
...
Each layer passes through `doppler secrets substitute` (Doppler secret resolution) and
`minijinja-cli` (strict Jinja templating; the schematic ID arrives as a `-D` define) before
`talosctl` merges them. Later patches strategically merge into earlier ones: documents with the same
kind/name are deep-merged, new documents are appended.
...
- **Secrets never live in this repo.** All sensitive values are Doppler `{{.VAR}}` references
...
just talos diff-node <node>            # diff live config against rendered target
